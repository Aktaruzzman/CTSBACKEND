...
module.exports = {
    ...
    findDriverByWhere: (wheres, wherenots) => {
        let query = db(tbl);
    ...
    deleteDriver: (id, status) => {
        if (status)
            this.updateDriver({ status: 'archived' }, id);
        else {
            let query = db(tbl);
            query = query.where('id', id);
